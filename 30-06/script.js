...
//Playlist
GET("https://edgemony-backend.herokuapp.com/playlist").then((playlist) => {
  playlist.map((playlist) =>
    createPlaylistEl(playlistEl, playlist.title, playlist.cover)
  );
});

//Film
GET("https://edgemony-backend.herokuapp.com/movies").then((movies) => {
  movies.map((movies) =>
    createPlaylistEl(playlistEl, movies.title, movies.poster)
  );
});
