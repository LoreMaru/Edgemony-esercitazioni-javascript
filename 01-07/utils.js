...
const createPlaylistEl = (parent, title, photo) => {
  const wrapper = c("div");
  const img = c("img");
  const playlistTitle = c("p");

  wrapper.className = "playlistCard";
  img.setAttribute("alt", title);
  img.setAttribute("src", photo);
  playlistTitle.textContent = title;

  wrapper.append(img, playlistTitle);
  parent.appendChild(wrapper);
};

export { c, q, createCard, createFriendEl, createMessageEl, createPlaylistEl };
